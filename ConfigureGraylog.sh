...
if [ "$SECURESSL" = "TRUE"  ]; then
   if [ -n "$CERTCONFURL" ]; then
    ...
    if [[ -n "$TRUSTSTORE_PASSWORD" && -e "$TRUSTSTORE_ALIAS" ]] ; then
        echo "Ok, we can generate: graylog.key &  graylog.crt"

        cd /opt/graylog/conf/nginx/ca/
        /opt/graylog/embedded/jre/bin/keytool  -importkeystore -srckeystore restcomm-combined.jks -destkeystore keystore.p12 -deststoretype PKCS12 -srcalias ${TRUSTSTORE_ALIAS}  -deststorepass ${TRUSTSTORE_PASSWORD}  -destkeypass ${TRUSTSTORE_PASSWORD} --srcstorepass ${TRUSTSTORE_PASSWORD}
        openssl pkcs12 -in keystore.p12   -nokeys   -out graylog.crt -passin pass: ${TRUSTSTORE_PASSWORD}
        openssl pkcs12 -in keystore.p12   -nocerts -nodes  -out graylog.key -passin ${TRUSTSTORE_PASSWORD}
    else
